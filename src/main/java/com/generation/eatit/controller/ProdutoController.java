...
public class ProdutoController {
	...
	public ResponseEntity<Produto> findByID(@PathVariable Long id) {
		return produtoRepository.findById(id).map(r -> ResponseEntity.ok(r)).orElse(ResponseEntity.notFound().build());
	}
	...
	public ResponseEntity<List<Produto>> findHigherValor(@PathVariable BigDecimal valor) {
		return ResponseEntity.ok(produtoRepository.findByValorGreaterThanOrderByValor(valor));
	}

	@GetMapping("/menorque/{valor}")
	public ResponseEntity<List<Produto>> findLowerValor(@PathVariable BigDecimal valor) {
		return ResponseEntity.ok(produtoRepository.findByValorLessThanOrderByValorDesc(valor));
	}

	@GetMapping("/valor/de{valorMenor}a{valorMaior}")
	public ResponseEntity<List<Produto>> findValorBetween(@PathVariable BigDecimal valorMenor,
			@PathVariable BigDecimal valorMaior) {
	...
	public ResponseEntity<Produto> postProdutos(@Valid @RequestBody Produto produto) {
		return produtoService.prodValid(produto).map(r -> ResponseEntity.status(HttpStatus.CREATED).body(r))
	...
	public ResponseEntity<Produto> putProdutos(@Valid @RequestBody Produto produto) {
		...
		return produtoService.prodatt(produto)
				.map(r -> ResponseEntity.status(HttpStatus.OK).body(produtoRepository.save(produto)))
	...
	public ResponseEntity<?> deleteProduto(@PathVariable Long id) {
		return produtoRepository.findById(id).map(r -> {
